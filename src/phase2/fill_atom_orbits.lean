...
| (0, a) := a
| (n + 1, a) := (w.orbit_set_equiv (w.litter_perm.symm L)).symm (inl (n, a))
...
lemma next_forward_image_eq {L₁ L₂ : litter} {a b : ℕ × w.need_forward_images}
  (hL₁ : L₁ ∈ w.litter_map.dom) (hL₂ : L₂ ∈ w.litter_map.dom)
...
begin
  ...
  rw h at ha,
  refine w.litter_perm.inj_on (or.inl (or.inl hL₁)) (or.inl (or.inl hL₂)) _,
...
begin
  ...
  { rw subtype.coe_eq_iff at h,
    obtain ⟨h₁, h₂⟩ := h,
    have h₁' := ((w.orbit_set_equiv (w.litter_perm.symm L₂)).symm (inl (n, b))).prop,
    have := eq_of_mem_litter_set_of_mem_litter_set
      (w.orbit_set_subset _ h₁).1 (w.orbit_set_subset _ h₁').1,
    rw [local_perm.eq_symm_apply, local_perm.right_inv] at this,
    exact this,
    exact or.inl (or.inl hL₁),
    exact w.litter_perm.symm.map_domain (or.inl (or.inl hL₁)),
    exact or.inl (or.inl hL₂), },
end

lemma next_forward_image_injective {L : litter} {a b : ℕ × w.need_forward_images}
  (hL : L ∈ w.litter_map.dom)
...
begin
  ...
  exact w.next_forward_image_injective hL₁ h,
end
...
begin
  ...
  exact w.next_backward_image_injective ha hb hL₁ h,
end
...
begin
  ...
  { rw [next_forward_image, next_backward_image],
    ...
    rw h at this,
    have := eq_of_mem_litter_set_of_mem_litter_set this
...
noncomputable def next_image (a : atom) (L : litter) (ha : a ∈ w.orbit_set L) : atom :=
(w.orbit_set_equiv L ⟨a, ha⟩).elim (w.next_backward_image L) (w.next_forward_image L)

def next_image_domain : set atom :=
⋃ L ∈ w.litter_map.dom, coe '' {a : w.orbit_set L | (w.orbit_set_equiv L a).elim
...
small.bUnion w.litter_map_dom_small
  (λ L hL, small.image (lt_of_le_of_lt (cardinal.mk_subtype_le _) (w.orbit_set_small L)))

lemma litter_map_dom_of_mem_next_image_domain {a : atom} (h : a ∈ w.next_image_domain) :
  (w.litter_map a.1).dom :=
begin
  rw next_image_domain at h,
...
lemma mem_orbit_set_of_mem_next_image_domain {a : atom} (h : a ∈ w.next_image_domain) :
  a ∈ w.orbit_set a.1 :=
begin
  rw next_image_domain at h,
  simp only [pfun.mem_dom, Union_exists, mem_Union, mem_image, mem_set_of_eq, set_coe.exists,
...
lemma orbit_set_equiv_elim_of_mem_next_image_domain {a : atom} (h : a ∈ w.next_image_domain) :
  (w.orbit_set_equiv a.1 ⟨a, w.mem_orbit_set_of_mem_next_image_domain h⟩).elim
...
lemma next_image_injective (a b : atom)
  ...
  (h : w.next_image a a.1 (w.mem_orbit_set_of_mem_next_image_domain ha) =
    w.next_image b b.1 (w.mem_orbit_set_of_mem_next_image_domain hb)) : a = b :=
begin
  rw [next_image, next_image] at h,
  obtain ⟨a', ha'⟩ := (w.orbit_set_equiv a.fst).symm.surjective
    ⟨a, w.mem_orbit_set_of_mem_next_image_domain ha⟩,
  obtain ⟨b', hb'⟩ := (w.orbit_set_equiv b.fst).symm.surjective
    ⟨b, w.mem_orbit_set_of_mem_next_image_domain hb⟩,
  have hae := w.orbit_set_equiv_elim_of_mem_next_image_domain ha,
  have hbe := w.orbit_set_equiv_elim_of_mem_next_image_domain hb,
  ...
  { cases w.next_backward_image_injective' _ _ _ _ h,
    ...
    { exact hae, },
    { exact hbe, },
    { exact w.litter_map_dom_of_mem_next_image_domain ha, },
    { exact w.litter_map_dom_of_mem_next_image_domain hb, }, },
...
λ a, {
  ...
  get := λ h, or.elim' h
    (w.atom_map a).get
    (w.next_image a a.1 ∘ w.mem_orbit_set_of_mem_next_image_domain)
}
...
begin
  rw set.disjoint_iff,
  rintros a ⟨ha₁, h₂⟩,
  exact (w.orbit_set_subset _ (w.mem_orbit_set_of_mem_next_image_domain h₂)).2 (or.inl ha₁),
end
...
lemma orbit_atom_map_eq_of_mem_next_image_domain (a : atom) (ha : a ∈ w.next_image_domain) :
  (w.orbit_atom_map a).get (or.inr ha) =
    w.next_image a a.1 (w.mem_orbit_set_of_mem_next_image_domain ha) :=
...
begin
  obtain ⟨b', hb'⟩ := (w.orbit_set_equiv b.fst).symm.surjective
    ⟨b, w.mem_orbit_set_of_mem_next_image_domain hb⟩,
  rw [orbit_atom_map_eq_of_mem_dom, orbit_atom_map_eq_of_mem_next_image_domain, next_image,
...
lemma next_image_atom_mem_litter_map (a : atom) (ha : a ∈ w.next_image_domain)
  (hdiff : ∀ L hL, ((w.litter_map L).get hL : set atom) ∆ litter_set ((w.litter_map L).get hL).1 ⊆
    w.atom_map.ran) :
  w.next_image a a.fst (w.mem_orbit_set_of_mem_next_image_domain ha) ∈
    (w.litter_map a.fst).get (w.litter_map_dom_of_mem_next_image_domain ha) :=
begin
  have hL := w.litter_map_dom_of_mem_next_image_domain ha,
  have := w.mem_orbit_set_of_mem_next_image_domain ha,
  obtain ⟨a', ha'⟩ := (w.orbit_set_equiv a.fst).symm.surjective
    ⟨a, w.mem_orbit_set_of_mem_next_image_domain ha⟩,
  have := w.orbit_set_equiv_elim_of_mem_next_image_domain ha,
  rw [next_image],
  rw [← ha', equiv.apply_symm_apply] at this ⊢,
  sorry
end

lemma next_image_atom_mem (a : atom) (ha : a ∈ w.next_image_domain)
  (L : litter) (hL : (w.litter_map L).dom) :
  a.fst = L ↔
    w.next_image a a.fst (w.mem_orbit_set_of_mem_next_image_domain ha) ∈ (w.litter_map L).get hL :=
begin
  sorry,
end

lemma orbit_atom_mem (a : atom) (ha : (w.orbit_atom_map a).dom)
  (L : litter) (hL : (w.litter_map L).dom) :
...
begin
  ...
  { rw orbit_atom_map_eq_of_mem_next_image_domain,
    exact w.next_image_atom_mem a ha L hL, },
end

noncomputable def fill_atom_orbits : weak_near_litter_approx := {
