...
private lemma technical_lemma  (x: ↥(litter_set N.fst)) (hx: (inl x.val, A) ∉ σ.val.range) (hM_sd: ↥(litter_set N.fst ∆ ↑(N.snd)) → atom)
(hM3: ∀ (a : ↥(litter_set N.fst ∆ ↑(N.snd))), (inl (↑a, hM_sd a), A) ∈ (σ.val)⁻¹) : x.val ∈ N.snd.val := begin
          suffices : x.val ∉ litter_set N.fst ∆ N.snd.val,
          dsimp [(∆)] at this, rw [← subtype.val_eq_coe, (eq_true_intro x.property)] at this,
...
lemma cond_atom_of_range_atom (y : atom) (h : (inl y, A) ∈ σ.val.range) : ∃ ( a : atom), (⟨sum.inl (a,y), A⟩ : binary_condition B)∈ σ.val := begin
...
private lemma useful {a : atom} {L' : near_litter} {N : near_litter} (hM_sd: ↥(litter_set N.fst ∆ ↑(N.snd)) → atom) (hM4: litter_set a.fst = ↑(L'.snd) ∆ range hM_sd) : a.fst = L'.fst :=
begin
          have := L'.2.property,
          ...
          dsimp [(is_near)], rw hM4, rw ← symm_diff_assoc,
          apply lt_of_le_of_lt, apply cardinal.mk_le_mk_of_subset, exact symm_diff_le_sup (  ↑(L'.snd) ∆ litter_set L'.fst) (range hM_sd),
          apply lt_of_le_of_lt, apply cardinal.mk_union_le, rw symm_diff_comm, apply cardinal.add_lt_of_lt κ_regular.aleph_0_le L'.snd.property,
          exact lt_of_le_of_lt cardinal.mk_range_le N.snd.property,
...
begin
 ...
 { by_cases N.fst = L,
  ...
  rw sup_inv,
   ...
   {classical,
    have := σ.property.backward.near_litter_cond _ _ A ha,
    obtain ⟨M, hM1, hM_sd, hM3, hM4 ⟩ := this,
    by_cases h2: L = N.fst,
    {
      subst h2,
        have := (σ.property.forward.one_to_one A).near_litter _ hL hM1, dsimp only at this,
        subst this,
      refine spec.atom_cond.all L' (or.inl hL) (λ x : litter_set N.fst, dite ((inl x.val, A) ∈ σ.val.range)
          (λ hx, atom_value σ⁻¹ A x hx)
          (λ hx, ((atom_map σ a A N hsmall ha).inv_fun ⟨x.val, _, hx⟩).val))
        (λ y hy, _) (ext $ λ x, ⟨λ hx, _, λ hx, _⟩),
      exact technical_lemma σ A N x hx hM_sd hM3,
        by_cases h3: (inl y, A) ∈ σ.val.range,
     {
        ...
        },
        {
          ...
          have := (((atom_map σ a A N hsmall ha).symm) ⟨y, _⟩).property, simp only [subtype.val_eq_coe, mem_domain, not_exists, not_and, mem_sep_eq] at this,
          exact this,
          simp only, exact h3,
        },
        by_cases h4 : (inl x, A) ∈ σ.val.domain,
        {
          ...
          rw dif_pos,
          apply (σ⁻¹.property.backward.one_to_one cond_snd).atom, simp only [subtype.val_eq_coe, mem_set_of_eq],
        ...
        by_contra hx_2,
        rw useful at hM4,
        rw hM4 at hx_2, rw symm_diff_def at hx_2, simp only [sup_eq_union, mem_union_eq, mem_diff, set.mem_range, set_coe.exists, not_exists] at hx_2, push_neg at hx_2,
        ...
        have := ((atom_map σ a A N hsmall ha) ⟨x, _, h4⟩).property,
        {simp only [subtype.val_eq_coe, spec.mem_range, not_exists, not_and, mem_sep_eq] at this,
        suffices h5: ↑((atom_map σ a A N hsmall ha) ⟨x, _⟩) ∉ (litter_set N.fst) ∆ N.snd,
        rw symm_diff_def at h5, simp only [sup_eq_union, mem_union_eq, mem_diff] at h5, push_neg at h5,
        ...
        apply ((atom_map σ a A N hsmall ha) ⟨x, _⟩).property.2,-- ⋆
        ...
        by_cases h4 : @has_mem.mem ((atom ⊕ near_litter) × extended_index ↑B) (unary_spec ↑B) set.has_mem (inl ↑x_2_fst, A) (@coe (subtype (spec.allowable B)) (spec ↑B) coe_to_lift σ).range,
         {
          ...
          exact (@hmaps x_2_fst x begin subst hx_2, exact this end).mp x_2_fst.property,
        },
        {
          ...
          suffices : ↑aaa ∈ L'.snd.val, rw haaa at this, exact this,
          have := aaa.property.left,
          simp only [subtype.val_eq_coe] at hM4,
          ...
          have this2 := aaa.property.right, simp only [subtype.val_eq_coe, mem_domain, not_exists, not_and] at this2,
          exfalso,
          have := this2 _ this_1,
    ...
    },
   {
      refine spec.atom_cond.small_in L' (or.inl hL) _ _,
    {
        ...
        {
          ext, simp only [spec.mem_range, mem_sep_eq, mem_union_eq],
          rw and_or_distrib_left,
        },
        rw this, clear this,
        apply lt_of_le_of_lt, apply cardinal.mk_union_le,
        apply cardinal.add_lt_of_lt κ_regular.aleph_0_le,
        ...
        refine @cardinal.mk_le_mk_of_subset _ _ (N.2.1 \ (litter_set N.1)) _,
        intros x hx, simp only [spec.mem_range, mem_sep_eq] at hx,
        ...
        apply lt_of_le_of_lt, refine @cardinal.mk_le_mk_of_subset _ _ ((litter_set N.fst) ∆ N.snd.val) _,
        {rw symm_diff_def, simp only [sup_eq_union, subset_union_right],},
        exact (N.snd.property),
        },
      { intros a_1 a_2 h,
      cases h,
      exact hmaps h,
      ...
      have := y.property, simp only [subtype.val_eq_coe, mem_domain, not_exists, not_and, mem_sep_eq] at this,
