...
begin
  ...
  { by_cases a.fst ≠ L ∨ A ≠ C,
    { refine spec.atom_cond.small_out _ _,
      ...
      { rw (new_atom_conds σ a A N hsmall hc).image_domain',
       ext,
      ...
      have h2: (new_atom_conds σ a A N hsmall ha).carrier = range (atom_to_cond σ a A N hsmall ha),
      refl,
      rw h2 at this,
      rw this,
        ...
        have : {a_1 ∈ litter_set L | (inl a_1, C) ∈
          (σ.val ⊔ σ.new_atom_conds a A N hsmall ha).domain} = {a_1 ∈ litter_set L | (inl a_1, C) ∈
              σ.val.domain} ∪ {a_1 ∈ litter_set L | (inl a_1, C) ∈
                binary_condition.domain '' (new_atom_conds σ a A N hsmall ha).carrier},
       { rw (new_atom_conds σ a A N hsmall hc).image_domain', ext,
  ...
  { by_cases N.fst = L,
    { have :  σ.new_atom_conds a A N hsmall ha = {carrier := range (atom_to_cond σ a A N hsmall ha), domain := _ , range := _, image_domain' := _, image_range' := _},
      ...
      {rintro ⟨h⟩, exact hL h,
        ...
        {ext, simp only [subtype.val_eq_coe, mem_domain, not_exists, not_and, mem_sep_eq, mem_union_eq, spec.mem_range, mem_image,
          ...
          { rintro ⟨hL, h | h⟩,
            ...
            have aaa : a_1 ∈ {a' ∈ litter_set a.fst | (inl a', A) ∉ σ.val.domain} := _,
            split, exact hL, use atom_to_cond σ a A N hsmall ha ⟨a_1, aaa⟩,
            use a_1, use h_1, exact h_2,
            simp only [subtype.val_eq_coe, mem_domain, not_exists, not_and, mem_sep_eq], exact h_1,
            },
          { rintro (⟨hL, h⟩ | ⟨hL, h⟩),
            { exact ⟨hL, or.inl h⟩ },
            split, apply hL,
            ...
            use a_2, use h_1_h,  rw ← h_2, cases a_1, cases a_1_fst,cases a_1_fst,
            simp only [binary_condition.range_mk, map_inl, prod.mk.inj_iff], simp only [prod.mk.inj_iff] at h_1_2,
            exact ⟨h_1_2.left.right, h_1_2.right⟩,
            simp only [prod.mk.inj_iff, false_and] at h_1_2, exfalso, exact h_1_2,
            }
           },
        rw this,
        clear this,
