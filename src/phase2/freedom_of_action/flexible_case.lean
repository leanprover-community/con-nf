...
abbreviation rough_bijection (σ : allowable_spec B) :=
{L : litter // flex L A ∧ (inr L.to_near_litter, A) ∉ (σ : spec B).domain} ≃
{L : litter // flex L A ∧ (inr L.to_near_litter, A) ∉ (σ : spec B).range}
...
def inv (bij : rough_bijection A σ) : rough_bijection A σ⁻¹ := bij.symm

@[simp] lemma inv_symm (bij : rough_bijection A σ) : bij.inv.symm = bij := by ext; refl

@[simp] lemma inv_inv (bij : rough_bijection A σ) : bij.inv.inv = bij := by ext; refl

lemma inv_coe (bij : rough_bijection A σ) (L) : bij.inv (bij L) = L := equiv.symm_apply_apply _ _

lemma small_of_not_mem_spec
  (L : {L : litter | flex L A ∧ (inr L.to_near_litter, A) ∉ (σ : spec B).domain}) :
  small {a ∈ litter_set L | (inl a, A) ∈ (σ : spec B).domain} :=
begin
  obtain ⟨N, h₁, atom_map, h₂, h₃⟩ | ⟨h₁, h₂⟩ | ⟨N, hN, h₁, h₂⟩ := σ.prop.forward.atom_cond L A,
  { cases L.prop.2 (inr_mem_domain h₁) },
  { exact h₂ },
  { exact h₁ },
end
...
lemma small_of_rough_bijection (bij : rough_bijection A σ)
  (L : {L : litter // flex L A ∧ (inr L.to_near_litter, A) ∉ (σ : spec B).domain}) :
  small {a ∈ litter_set (bij L) | (inl a, A) ∈ (σ : spec B).range} :=
begin
  obtain ⟨N, h₁, atom_map, h₂, h₃⟩ | ⟨h₁, h₂⟩ | ⟨N, hN, h₁, h₂⟩ :=
    σ.prop.backward.atom_cond (bij L) A,
  { exfalso,
  rw spec.mem_inv at h₁,
    obtain ⟨hC, hn⟩ := (bij L).prop,
    exact hn (inr_mem_range h₁) },
  { rwa spec.domain_inv at h₂ },
  { rwa spec.domain_inv at h₁ }
end
...
def inv {σ : allowable_spec B} {bij : rough_bijection A σ}
...
noncomputable def precise_atom_image {σ} (bij : rough_bijection A σ)
  {L : {L : litter | flex L A ∧ (inr L.to_near_litter, A) ∉ dom}}
  {L' : {L : litter | flex L A ∧ (inr L.to_near_litter, A) ∉ rge}}
...
lemma precise_atom_image_range {dom rge} (bij : rough_bijection A σ)
  {L : {L : litter | flex L A ∧ (inr L.to_near_litter, A) ∉ dom}}
  {L' : {L : litter | flex L A ∧ (inr L.to_near_litter, A) ∉ rge}}
  (abij : precise_atom_bijection L L')
  (atom_value : {a : litter_set L // (inl (a : atom), A) ∈ dom} → atom) :
...
begin
  ...
  { rintro ⟨b, hb⟩, dsimp only at hb,
    ...
    { rw ← hb,
      exact or.inl (abij.to_fun ⟨b, _⟩).prop } },
  rintro (⟨ha₁, ha₂⟩ | ⟨b, hb⟩),
  { rw mem_set_of at ha₂,
    set b := abij.inv_fun ⟨a, ha₁, ha₂⟩ with hb,
    refine ⟨⟨b, b.prop.left⟩, _⟩,
    dsimp only,
    split_ifs,
    { cases b.prop.right h },
...
def precise_litter_image (bij : rough_bijection A σ)
  (L : {L : litter | flex L A ∧ (inr L.to_near_litter, A) ∉ (σ : spec B).domain})
  (abij : precise_atom_bijection L (bij L)) : near_litter :=
⟨(bij L),
  range (λ (a : litter_set L),
    precise_atom_image bij abij (λ a, atom_value σ A a.1 a.2) a a.prop),
  begin
...
lemma precise_litter_image_not_mem (bij : rough_bijection A σ)
  (L : {L : litter | flex L A ∧ (inr L.to_near_litter, A) ∉ (σ : spec B).domain})
  (abij : precise_atom_bijection L (bij L)) :
  (inr (precise_litter_image bij L abij), A) ∉ (σ : spec B).range :=
begin
  intro h, rw spec.mem_range at h, obtain ⟨⟨_ | ⟨N, _⟩, C⟩, hc₁, hc₂⟩ := h; cases hc₂, clear hc₂,
  obtain ⟨M, hM₁, _⟩ :=
    σ.prop.backward.near_litter_cond _ _ A hc₁,
...
begin
  ...
  { have := congr_arg sigma.fst h,
    ...
    simp only [litter.to_near_litter, subtyp(e : spec B)_eq_coe, subtype.coe_inj] at this,
    exact this.symm },
  subst this,
  rw ← near_litte(r : spec B)_injective.eq_iff at h ⊢,
  ...
  { intro a,
    ...
    unfold precise_atom_image,
    classical,
    by_cases (inl (c : atom), A) ∈ (σ : spec B).domain,
    { simp_rw dif_pos h,
      rw dif_pos,
    ...
    { simp_rw dif_neg h,
      ...
      { exact ((abij L₁).to_fun ⟨c, c.2, h⟩).prop.2 } } },
  { intros b hb,
    have := h₁ ⟨b, hb⟩,
    refine ⟨⟨_, this⟩, _⟩,
    unfold precise_atom_image,
    classical,
    by_cases (inl b, A) ∈ (σ : spec B).domain,
    ...
    { simp_rw subtype.coe_mk, simp_rw dif_neg h,
      ...
      { rw subtype.coe_eta,
        ...
        rw subtype.coe_mk at this, convert this using 1, exact habij _ _ },
      { exact ((abij L₁).to_fun ⟨b, hb, h⟩).prop.2 } } }
end

lemma precise_litter_image_inj {bij : rough_bijection A σ}
  {L₁ L₂ : {L : litter | flex L A ∧ (inr L.to_near_litter, A) ∉ (σ : spec B).domain}}
  (abij₁ : precise_atom_bijection L₁ (bij L₁))
  (abij₂ : precise_atom_bijection L₂ (bij L₂)) :
...
def new_flex_litters (bij : rough_bijection A σ)
  (abij : ∀ L, precise_atom_bijection L (bij L)) : spec B := {
  carrier := {c | ∃ (L : {L : litter | flex L A ∧ (inr L.to_near_litter, A) ∉ (σ : spec B).domain}),
    c = (inr (L.1.to_near_litter, precise_litter_image bij L (abij L)), A)},
  domain := {c | ∃ (L : {L : litter | flex L A ∧ (inr L.to_near_litter, A) ∉ (σ : spec B).domain}),
...
def new_inverse_flex_litters (bij : rough_bijection A σ)
  (abij' : ∀ L, precise_atom_bijection (bij.inv L) L) : spec B :=
(new_flex_litters bij.inv (λ L, equiv.symm (abij' L)))⁻¹

@[simp] lemma mem_new_flex_litters (bij : rough_bijection A σ)
...
@[simp] lemma mem_new_inverse_flex_litters (bij : rough_bijection A σ)
...
lemma precise_litter_image_flex (bij : rough_bijection A σ)
  (L : {L : litter | flex L A ∧ (inr L.to_near_litter, A) ∉ (σ : spec B).domain})
  (abij : precise_atom_bijection L (bij L)) :
...
lemma flex_union_one_to_one
  ...
  spec.one_to_one_forward
    ((σ : spec B) ⊔ bij.new_flex_litters abij ⊔ bij.new_inverse_flex_litters abij') :=
begin
  refine λ C, ⟨_, _⟩,
  { rintro b a₁ ((ha₁ | ha₁) | ha₁) a₂ ha₂,
    { obtain ((ha₂ | ha₂) | ha₂) := ha₂,
      { exact (σ.prop.forward.one_to_one C).atom b ha₁ ha₂ },
  ...
  { obtain ⟨N', hN'₁, hN'₂⟩ := σ.prop.backward.near_litter_cond _ _ C hM₁,
    obtain ⟨L', hL'⟩ := hM₂, cases hL',
    cases (bij L').prop.right (inr_mem_domain hN'₁) },
  { obtain ⟨L', hL'⟩ := hM₂, cases hL',
    cases L'.prop.right (inr_mem_range hM₁) },
  { obtain ⟨L, hL⟩ := hM₁,cases hL,
  ...
  { obtain ⟨L₁, hL₁⟩ := hM₁, obtain ⟨L₂, hL₂⟩ := hM₂,
    simp only [binary_condition.inv_def, map_inr, prod.swap_prod_mk, subtyp(e : spec B)_eq_coe,
      prod.mk.inj_iff] at hL₂,
    cases hL₁, cases hL₂.1.2,
    exact precise_litter_image_eq_symm habij hL₂.1.1 },
  { obtain ⟨L₁, hL₁⟩ := hM₁, cases hL₁,
    have := L₁.prop.2, simp_rw [allowable_spe(c : spec B)_inv, domain_inv, spec.mem_range] at this,
    cases this ⟨_, hM₂, rfl⟩ },
  { obtain ⟨L₁, hL₁⟩ := hM₁, obtain ⟨L₂, hL₂⟩ := hM₂,
  ...
  { obtain ⟨L₁, hL₁⟩ := hM₁, obtain ⟨L₂, hL₂⟩ := hM₂,
    ...
    rw subtyp(e : spec B)_inj at this,
    cases this, refl }
end

lemma flex_union_atoms_eq (L : litter) (C : extended_index B) :
  {a ∈ litter_set L | (inl a, C) ∈
    ((σ : spec B) ⊔ bij.new_flex_litters abij ⊔ bij.new_inverse_flex_litters abij').domain} =
  {a ∈ litter_set L | (inl a, C) ∈ (σ : spec B).domain} :=
begin
  ext a,
...
lemma flex_union_atom_cond (L C) :
  spec.atom_cond
    ((σ : spec B) ⊔ bij.new_flex_litters abij ⊔ bij.new_inverse_flex_litters abij') L C :=
begin
  ...
  { obtain rfl | h := eq_or_ne A C,
    { by_cases flex L A,
      { refine spec.atom_cond.small_in _ (or.inl $ or.inr ⟨⟨L, h, h₁⟩, rfl⟩) _ _,
        ...
        { rintro ⟨c, hbc⟩, unfold precise_atom_image at hbc, dsimp only at hbc,
          ...
          { have := atom_value_spec _ _ _ hc, simp_rw subtype.coe_mk at hbc, rw hbc at this,
            rw (σ.prop.forward.one_to_one A).atom b hab this,
            exact c.prop },
          { have := ((abij ⟨L, h, h₁⟩).to_fun ⟨c, c.prop, hc⟩).prop,
            rw hbc at this,
      ...
      simp only [subtyp(e : spec B)_eq_coe, binary_condition.domain_mk, map_inr,
...
begin
  ...
  { rw set.mem_range at ha₂, push_neg at ha₂,
    have := ha₂ ⟨((abij' L).to_fun ⟨a, ha₁, h⟩() : spec B), ((abij' L).to_fun ⟨a, ha₁, h⟩).prop.1⟩,
    classical,
    rw dif_neg at this,
    { refine this _,
      generalize_proofs,
      simp only [subtype.coe_mk, equiv.to_fun_as_coe, subtyp(e : spec B)_eq_coe, subtype.coe_eta,
        equiv.symm_apply_apply] },
    exact ((abij' L).to_fun ⟨a, ha₁, h⟩).prop.2 },
  { obtain ⟨b, hb⟩ := ha₁,
    dsimp only at hb,
    ...
    { have := atom_value_spec σ⁻¹ A b h',
      ...
      exact h ⟨_, this, rfl⟩ },
    { rw ← hb at ha₂,
      cases ha₂ ((abij' L).symm ⟨b, b.2, h'⟩).prop.1 } },
end

lemma flex_union_near_litter_cond :
  ∀ N₁ N₂ C, spec.near_litter_cond
    ((σ : spec B) ⊔ bij.new_flex_litters abij ⊔ bij.new_inverse_flex_litters abij') N₁ N₂ C :=
begin
  rintro N₁ N₂ C ((h | h) | h),
  ...
  { rw [set_like.mem_coe, mem_new_inverse_flex_litters] at h,
    ...
    { simp only [subtyp(e : spec B)_eq_coe, prod.mk.inj_iff] at hL,
      sorry } }
end
...
begin
  ...
  rintro β γ δ hγ hδ hγδ C t ht π hsupp,
  rw [domain_sup, domain_sup] at ht hsupp,
  obtain ((ht | ht) | ht) := ht,
  { refine σ.prop.forward.support_closed hγ hδ hγδ C t ht _ _,
    intros a ha, exact hsupp (or.inl (or.inl ha)) },
  { exfalso,
    simp only [subtyp(e : spec B)_eq_coe, mem_domain, not_exists, not_and, mem_set_of_eq,
...
lemma flex_union_flex_cond (C) :
  spec.flex_cond B
    ((σ : spec B) ⊔ bij.new_flex_litters abij ⊔ bij.new_inverse_flex_litters abij') C :=
begin
  obtain rfl | h := eq_or_ne C A,
  { refine spec.flex_cond.all (λ L hL, _) (λ L hL, _),
    { by_cases h : ((inr L.to_near_litter, C) ∈ (σ : spec B).domain);
    ...
    { -- might want to switch naming around if it doesn't shorten code
      by_cases h : ((inr L.to_near_litter, C) ∈ (σ : spec B).range);
...
lemma forward_eq_backward :
  ((σ : spec B) ⊔ bij.new_flex_litters abij ⊔ bij.new_inverse_flex_litters abij')⁻¹ =
  σ⁻(¹ : spec B) ⊔ bij.inv.new_flex_litters (λ L, equiv.symm $ abij' L) ⊔
    bij.inv.new_inverse_flex_litters (λ L, equiv.symm $ abij L) :=
begin
...
lemma flex_union_allowable
  (habij : ∀ L a, (equiv.symm (abij' (bij L)) a() : spec B) = (equiv.symm (abij L) a() : spec B))
  (habij' : ∀ L a, ((abij (bij.inv L)).to_fun a() : spec B) = ((abij' L).to_fun a() : spec B)) :
  spec.allowable B
    ((σ : spec B) ⊔ bij.new_flex_litters abij ⊔ bij.new_inverse_flex_litters abij') :=
{ forward :=
...
lemma le_flex_union
  (habij : ∀ L a, (equiv.symm (abij' (bij L)) a() : spec B) = (equiv.symm (abij L) a() : spec B))
  (habij' : ∀ L a, ((abij (bij.inv L)).to_fun a() : spec B) = ((abij' L).to_fun a() : spec B)) :
  σ ≤ ⟨(σ : spec B) ⊔ bij.new_flex_litters abij ⊔ bij.new_inverse_flex_litters abij',
...
{ le := by { simp_rw sup_assoc, exact le_sup_left },
  all_flex_domain := λ L N' C hN' hσ₁ hσ₂ L' hL', begin
    ...
    { rw [spec.domain_sup, spec.domain_sup],
      by_cases (inr L'.to_near_litter, C) ∈ (σ : spec B).domain,
    ...
    { rw [spec.range_sup, spec.range_sup],
      by_cases (inr L'.to_near_litter, C) ∈ (σ : spec B).range,
      { exact or.inl (or.inl h) },
      { have : A = C,
        { obtain ((_ | ⟨L₁, hL₁⟩) | ⟨L₁, hL₁⟩) := hσ₂,
          { cases hσ₁ hσ₂ },
...
lemma exists_ge_flex (σ : allowable_spec B) {L : litter} (hL : flex L A) :
  ∃ ρ, σ ≤ ρ ∧ (⟨inr L.to_near_litter, A⟩ : support_condition B) ∈ (ρ : spec B).domain :=
begin
  by_cases (inr L.to_near_litter, A) ∈ (σ : spec B).domain,
  { exact ⟨σ, le_rfl, h⟩ },
  obtain ⟨hdom, hrge⟩ | ⟨hdom, hrge⟩ := σ.prop.flex_cond A,
  swap, { cases h (hdom L hL) },
  obtain ⟨bij⟩ :nonempty (rough_bijection A σ) :=
    cardinal.eq.1 (hdom.symm.trans hrge),
  have abij : ∀ L, precise_atom_bijection L (bij L),
  { refine λ L, (cardinal.eq.1 _).some,
    rw [mk_sep, mk_sep],
    transitivity #κ,
    { have := mk_sum_compl {a : litter_set L | (inl (a : spec B), A) ∈ (σ : spec B).domain},
    ...
    { have := mk_sum_compl
        {a : litter_set (bij L) | (inl (a : spec B), A) ∈ (σ : spec B).range},
  ...
  set abij' : ∀ L, precise_atom_bijection (bij.inv L) L := λ L, (abij (bij.inv L)).trans {
    to_fun := λ a, ⟨a,
      by simpa only [rough_bijection.inv, subtyp(e : spec B)_eq_coe, equiv.apply_symm_apply] using a.2.1,
      a.2.2⟩,
    inv_fun := λ a, ⟨a,
      by simpa only [rough_bijection.inv, subtyp(e : spec B)_eq_coe, equiv.apply_symm_apply] using a.2.1,
      a.2.2⟩,
  ...
  have habij₂ : ∀ L a, ((equiv.symm (abij' (bij L))).trans
    ((abij' (bij L)).trans (abij L).symm) a() : spec B) = (equiv.symm (abij L) a() : spec B),
  { intros L a, rw ← equiv.trans_assoc, rw equiv.symm_trans_self, refl },
  have habij : ∀ L a, (equiv.symm (abij' (bij L)) a() : spec B) = (equiv.symm (abij L) a() : spec B),
  { intros L a, rw [← habij₂, habij₁], refl },
  have habij' : ∀ L a, ((abij (bij.inv L)).to_fun a() : spec B) = ((abij' L).to_fun a() : spec B),
  { intros L a, refl },
