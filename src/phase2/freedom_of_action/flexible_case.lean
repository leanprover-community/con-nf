...
lemma precise_litter_inverse_image_not_mem (bij : rough_bijection A σ.val.domain σ.val.range)
  (L : {L : litter | flexible L A ∧ (inr L.to_near_litter, A) ∉ σ.val.range})
  (abij : precise_atom_bijection bij.symm L) :
  (inr (precise_litter_inverse_image bij L abij), A) ∉ σ.val.domain :=
begin
  intro h, rw spec.mem_domain at h, obtain ⟨⟨_ | ⟨_, N⟩, C⟩, hc₁, hc₂⟩ := h; cases hc₂, clear hc₂,
  obtain ⟨M, hM₁, _⟩ :=
    σ.property.forward.near_litter_cond _ _ A hc₁,
  cases (bij.symm L).prop.2 (inr_mem_domain hM₁),
end

lemma precise_litter_image_eq_symm
  {bij : rough_bijection A σ.val.domain σ.val.range}
  {abij : Π L, bij.precise_atom_bijection L}
  {L₁ L₂ : ↥{L : litter | flexible L A ∧ (inr L.to_near_litter, A) ∉ σ.val.domain}} :
  bij.precise_litter_image L₁ (abij L₁) = (bij L₂).val.to_near_litter →
  L₁.val.to_near_litter = bij.precise_litter_inverse_image (bij L₂) (abij L₂).symm :=
begin
  intro h,
  have : L₁ = L₂,
  { have := congr_arg sigma.fst h,
    exact (embedding_like.apply_eq_iff_eq _).mp (subtype.coe_inj.mp this), },
  subst this,
  rw ← near_litter.val_injective.eq_iff at h ⊢,
  rw [precise_litter_image, precise_litter_image_aux] at h,
  rw [precise_litter_inverse_image, precise_litter_image_aux],
...
def new_inverse_flexible_litters (bij : rough_bijection A σ.val.domain σ.val.range)
  ...
  image_range' := begin
    ext c, simp only [subtype.val_eq_coe, mem_domain, not_exists, not_and, mem_set_of_eq,
      set_coe.exists, subtype.coe_mk, mem_image],
    split,
    { rintro ⟨d, ⟨L, ⟨hL₁, hL₂⟩, rfl⟩, rfl⟩, refine ⟨⟨L, hL₁, _⟩, rfl⟩,
      intro h, rw mem_domain at h, obtain ⟨c, hc₁, hc₂⟩ := h, cases hL₂ c hc₁ hc₂, },
    { rintro ⟨⟨L, hL₁, hL₂⟩, rfl⟩, refine ⟨_, ⟨L, ⟨hL₁, _⟩, rfl⟩, rfl⟩,
      rw mem_domain at hL₂, push_neg at hL₂, exact hL₂, }
  end,
}
...
begin
  ...
  { obtain ⟨L₁, hL₁⟩ := hM₁, obtain ⟨L₂, hL₂⟩ := hM₂,
    ...
    exact precise_litter_image_eq_symm hL₂.1.2, },
  { obtain ⟨L₁, hL₁⟩ := hM₁, cases hL₁,
    have := (bij L₁).prop.2, rw spec.mem_range at this, cases this ⟨_, hM₂, rfl⟩, },
  { obtain ⟨L₁, hL₁⟩ := hM₁, obtain ⟨L₂, hL₂⟩ := hM₂,
    rw [prod.mk.inj_iff, sum.inr.inj_iff, prod.mk.inj_iff] at hL₂, cases hL₁, cases hL₂.1.1,
    exact (precise_litter_image_eq_symm hL₂.1.2.symm).symm, },
  { obtain ⟨L₁, hL₁⟩ := hM₁, obtain ⟨L₂, hL₂⟩ := hM₂,
