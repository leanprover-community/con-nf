...
def precise_atom_bijection
  ...
  (L₂ : {L : litter | flexible L A ∧ (inr L.to_near_litter, A) ∉ rge}) :=
...
begin
  ...
  { intro a,
    ...
    { simp_rw dif_pos h,
      rw dif_pos,
      { refine atom_value_eq_of_mem _ _ _ _ _,
...
(new_flexible_litters bij.inv (λ L, equiv.symm (abij' L)))⁻¹
...
begin
  unfold new_inverse_flexible_litters new_flexible_litters,
  split,
...
begin
  ...
  { rintro b a₁ ((ha₁ | ha₁) | ha₁) a₂ ha₂,
    { obtain ((ha₂ | ha₂) | ha₂) := ha₂,
      ...
      { simpa only [set_like.mem_coe, mem_new_inverse_flexible_litters, prod.mk.inj_iff,
  ...
  { obtain ⟨L₁, hL₁⟩ := hM₁, cases hL₁,
    have := L₁.prop.2, simp_rw [allowable_partial_perm.val_inv, domain_inv, spec.mem_range] at this,
    cases this ⟨_, hM₂, rfl⟩, },
  { obtain ⟨L₁, hL₁⟩ := hM₁, obtain ⟨L₂, hL₂⟩ := hM₂,
    ...
    { by_cases flexible L A,
      { refine spec.atom_cond.small_in _ (or.inl $ or.inr ⟨⟨L, h, h₁⟩, rfl⟩) _ _,
        ...
        { intro ha, refine ⟨⟨a, ha⟩, _⟩,
          unfold precise_atom_image,
          rw dif_pos _,
          exact atom_value_eq_of_mem _ _ _ _ hab,
...
lemma new_flexible_litters_inv :
  (bij.new_flexible_litters abij)⁻¹ =
  bij.inv.new_inverse_flexible_litters (λ L, equiv.symm $ abij L) :=
begin
  ext ⟨_ | ⟨N₁, N₂⟩, C⟩,
...
lemma new_inverse_flexible_litters_inv :
  (bij.new_inverse_flexible_litters abij')⁻¹ =
  bij.inv.new_flexible_litters (λ L, equiv.symm $ abij' L) :=
begin
...
lemma forward_eq_backward :
  (σ.val ⊔ bij.new_flexible_litters abij ⊔ bij.new_inverse_flexible_litters abij')⁻¹ =
  σ⁻¹.val ⊔ bij.inv.new_flexible_litters (λ L, equiv.symm $ abij' L) ⊔
    bij.inv.new_inverse_flexible_litters (λ L, equiv.symm $ abij L) :=
begin
  rw [sup_inv, sup_inv, sup_assoc, sup_assoc, new_flexible_litters_inv,
...
lemma flexible_union_allowable
  ...
  spec.allowable B
    (σ.val ⊔ bij.new_flexible_litters abij ⊔ bij.new_inverse_flexible_litters abij') :=
{ forward :=
  { one_to_one := flexible_union_one_to_one bij abij abij' habij,
    atom_cond := flexible_union_atom_cond bij abij abij',
    near_litter_cond := flexible_union_near_litter_cond bij abij abij',
    non_flexible_cond := flexible_union_non_flexible_cond bij abij abij',
  ...
  { one_to_one :=
    ...
    atom_cond :=
      by { convert flexible_union_atom_cond bij.inv _ _,
        rw forward_eq_backward bij abij abij' },
    near_litter_cond :=
      by { convert flexible_union_near_litter_cond bij.inv _ _,
        rw forward_eq_backward bij abij abij' },
    non_flexible_cond :=
      by { convert flexible_union_non_flexible_cond bij.inv _ _,
        rw forward_eq_backward bij abij abij' },
    support_closed :=
      by { convert flexible_union_support_closed bij.inv _ _,
        rw forward_eq_backward bij abij abij' },
  },
  flexible_cond := flexible_union_flexible_cond bij abij abij',
}
