...
instance allowable_group_pi' (hβ : β ≤ α) (γ : Λ) :
...
instance allowable_group_pi (hβ : β ≤ α) : group (Π γ (h : γ < β), allowable γ (h.trans_le hβ)) :=
...
lemma semiallowable_perm_scalar_def {β : Λ} (hβ : β ≤ α) (π : semiallowable_perm α hβ)
  (X : code α β hβ) :
  π • X =
    ⟨X.extension, X.extension_lt,
    option.rec
...
⟨λ π X, ⟨π • X.val, begin
  obtain ⟨⟨γ, hγ, G⟩, hG⟩ := X,
  rw semiallowable_perm_scalar_def,
...
lemma allowable_perm_coherence {β γ δ : Λ} {hβ : β ≤ α} (π : allowable_perm α hβ) (hγ : γ < β)
  (hδ : δ < β) (g) :
...
lemma allowable_perm_commute {β : Λ} {hβ : β ≤ α} (π : allowable_perm α hβ) {δ : Λ} (hδ : δ < β)
...
lemma representative_code_map {β : Λ} {hβ : β ≤ α} (π : allowable_perm α hβ) {δ : Λ} (hδ : δ < β)
