...
\begin{lemma}
\label {lem:count-near-litters} \uses {def:kappa}  \uses {def:mu}  \uses {def:near-litter}
...
\begin{definition}
\label {def:alpha} \uses {def:proper-type-index}
...
\begin{definition}
\label {def:codes}  \uses {def:alpha}  \uses {def:oldtau}
...
\begin{lemma}
\label {lem:near-litter-code} \uses {def:near-litter}  \uses {def:codes}  \uses {def:oldtau}
...
\begin{definition}
\label {def:alt-ext}  \uses {def:fmap} \uses {def:codes}
...
\begin{definition}
\label {def:representative-code}  \uses {def:equiv-code}
...
\begin{definition}
\label {def:ttt-membership}  \uses {def:representative-code}  \uses {def:ainverse}  \uses {def:codes}
...
\label {thm:extensionality}  \uses {ttt-membership}
...
\label {def:structural}  \uses {def:codes}
...
\begin{definition}
\label {def:derivative}  \uses {def:structural}
...
\label {def:allowable}  \uses {def:structural}
...
\label {lem:coherence-unpacked}  \uses {def:allowable}
...
\label {rep-preserved}  \uses {coherence-unpacked}
...
\label {support}  \uses {extended-type-index}
...
$((\gamma,-1,X),A,\delta)$, where $A$ is a proper extended type index whose minimum is $\gamma$ and whose maximum is $\beta$, $\delta$ is a proper type index $\leq \beta$ (whose function will not be apparent at all at this point)
...
\label {support-of}  \uses {allowable}
...
we have for each $(x,A,\beta) \in {\tt dom}(S)$ [yes, we really mean the third component is $\beta$] that $\pi_A(x)=x$, we also have that $\pi(X)=X$.

\end{definition}

\begin{definition}

\label {tau-alpha} \uses {support-of}
