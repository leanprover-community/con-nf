...
Define $\tau_{-1}$ as your favorite set of size $\mu$.  It is convenient to suppose that elements of $\tau_{-1}$ are ordered triples with first component $-1$;  we would not care in the least what their other components were.
...
We define $\Lambda$ as your favorite partition of $\tau_{-1}$ into subsets of size $\kappa$.  We say that a set $L$ is a litter iff it is an element of $\Lambda$.

\end{definition}

\begin{definition} 
\label{def:local_cardinal}
A set $K$ is a local cardinal iff there is a litter $L$ such that $K=\{N \subseteq \tau_{-1}:|N \Delta L|<\kappa\}$.
\end{definition}
\begin{definition}
\label{def:near_litter} %\uses{def:local_cardinal}
