...
\begin{itemize}
  ...
  \item an injection $j_\beta$ from the set of local cardinals (equivalently, the set litters) to $\tau_\beta$. (We will call these the \emph{typed litters} below.
\end{itemize}
...
Equivalently (this may be easier for the formalisation), the relation $\rightsquigarrow$ on codes --- defined by $ (\beta,\gamma ,X) \rightsquigarrow A_\gamma,\delta (\beta,\gamma ,X)$ for all codes $ (\beta,\gamma ,X)$ and all $\delta \neq \gamma$ --- is well-founded.
\end{lemma}

\begin{proof}
  Consider the map $j$ from codes to $\mu$ sending $(\beta,\gamma,X)$ to $\min \iota_\gamma``X$, where $\iota_\gamma : \tau_\gamma \to \mu$ is the  position function assumed in the stage 1 data.

  The definition of the functions $A_{\delta,\gamma}$ and the condition that $\iota f_{\gamma,\delta}()$ ensure that $j(\beta,\gamma,X) < j(A_\delta(\beta,\gamma,X))$, for all suitable $\gamma,\delta,X$.
...
If $(\beta,\gamma,G)$ has odd height then $(\beta,\delta,D) \equiv_\beta (\beta,\gamma,G)$ iff $(\beta,\delta,D) \equiv_\beta A^{-1}(\beta,\gamma,G)$ (which reduces to the previous case).
\end{definition}

\begin{definition}
\label {def:representative-code}
\uses{def:equiv-code}
Note that each equivalence class under $\equiv_\beta$ with nonempty third component contains exactly one element with even height.  We refer to this code as the representative code of its equivalence class, and we refer to such codes as representative codes generally, with the additional stipulation that $(\beta,-1,\emptyset)$ is a representative code as well.
\end{definition}

\begin{lemma}
\label {lem:objects-are-representative}
