...
The bluntest approach, which should certainly suffice for everything, is to assume some form of “at every level, the tangle data is equal to what is constructed from the data at earlier levels, but the constructions of phase 1”.  This is made precise in “coherent tangle data” \cref{def:coherent-tangle-data}.  However, this is somewhat awkward to work with in formalisation.  A more cleanly-structured version is given by “freedom of action contexts”, \cref{def:foa-context}, which suffices at least for strong supports and freedom of action.  Hopefully it may also be possible to recast counting tangles in terms of \cref{def:foa-context} or some other algebraically-cleaner version, but we haven’t done so yet.
...
\begin{definition}
  ...
  Taking inverse images under these maps, paths act contravariantly on sets of binary conditions: a path $A : \beta \to \alpha$ induces $(-)_A : \pow(\BiCond_\alpha) \to \po(\BiCond(\beta))$, given by $\sigma_A := \{ (x,y,B) \mid (x,y,BA) \in \sigma \}$.
\end{definition}
...
\begin{proposition}
  ...
  (NOTE: sthis may not be explicitly needed, but it is helpful to know, and should be a good warmup and definition-test ahead of the main freedom of action theorem.)
\end{proposition}
...
\begin{definition}
  ...
  \item (Atoms:) For each litter $(L,A)$ not in $\dom \sigma$, then $\dom \sigma$ contains $<\kappa$-many atoms $(x,A)$ with $x \in L$; and dually for $\rge(\sigma)$.  On the other hand, for each litter $(L,A) \i \dom \sigma$ (say $M$ is the unique near-litter such that $(L,M,A) \in \sigma$), then either $\dom \sigma$ contains $<\kappa$-many atoms of $(L,A)$ and $\rge \sigma$ contains $<\kappa$-many atoms of $(M,A)$, or else $\dom \sigma$ contains all atoms of $(L,A)$ and all except $<\kappa$-many of them are mapped to atoms in $(M,A)$, and dually, $\rge \sigma$ contains all atoms of $(M,A)$ and maps them almost all to $(L,A)$; and dually for each litter $(L,A) \in \rge \sigma$.
...
\begin{proposition}
  \label{def:allowability-restriction}
...
\begin{definition}
  ...
  Most constructions in phase 2 will assume coherent tangle data below $\alpha$, for some fixed $\alpha$.

  [Terminology: perhaps we should call this “phase 2 data at $\alpha$”?  That seems more consistent for the overall naming scheme, but “coherent tangle data below $\alpha$” seems clearer and more transparent in itself.  Maybe we should reconsider the naming scheme, so that it’s not in tension with clarity!]
\end{definition}
...
Throughout this section, fix some $\alpha \in \lambda + 1$, and assume coherent tangle data below $\alpha$. [Actually, perhaps this section doesn’t need the coherence and could be done in phase 1, i.e. working purely in a single level, assuming full tangle data is available at lower levels?  This should become clearer as it is formalised.]
...
\uses{def:coherent-tangle-data,def:raise-lower-index}
...
\begin{definition}
  ...
  \uses{def:coherent-tangle-data}
  Fix coherent tangle data below $\alpha$.  A \emph{support-specification} is \ldots [this is quite long to define; we should get the text from more recent versions of Randall’s note]
...
\begin{lemma}
  ...
  \uses{def:freedom-of-action-statement-old}
  Given coherent tangle data below $\alpha$ satisfying freedom of action at $\alpha$, we can choose supports $S(x)$ for all $\alpha$-tangles $x$ and a position function $\iota_\alpha : \tau_\alpha \to \mu$ satisfying the conditions demanded in \cref{def:tangle-data}:
  \begin{enumerate}
