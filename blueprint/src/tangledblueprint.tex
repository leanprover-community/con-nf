...
\begin{definition}
  ...
  Say a set of binary conditions $\sigma \subseteq \BiCond_\alpha$ is \emph{total} if its domain is $\Cond_\alpha$; co-total if its range is; and 1-1 if whenever $(x,y,A)$ and $(x',y',A')$ are in $\sigma$, $x = x'$ if and only if $y = y'$.
\end{definition}

\begin{proposition}
  \label{prop:total-1-1-gives-perm}
  \uses{def:condition-total-1-1,def:conditions-from-perm}
  A set of binary conditions is the graph of a structural permutation precisely if it is 1-1 and total.

  (NOTE: this may not be explicitly needed, but it is helpful to know, and should be a good warmup and definition-test ahead of the main freedom of action theorem.)
\end{proposition}
