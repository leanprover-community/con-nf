...
We define, for all $\beta,\gamma < \alpha$, with $\gamma$ proper, a map $f_{\beta,\gamma}$ from $\tau_\beta$ to $\mathrm{Litter}$, as follows.
...
\uses{def:alt-ext, lem:fmap-properties}
\begin{enumerate}
...
\begin{definition}
  \label{def:structural-perms-atoms}
  \uses{def:atoms}
  \leanok
...
\begin{definition}
  \label{def:structural-perms}
  \uses{def:structural-perms-atoms}
  %leanok
  For a \emph{proper} type index $\alpha$, the structural $\alpha$-permutations are the group $\Str_\alpha = \prod_{\beta < \alpha} \Str_\beta$.
\end{definition}

\begin{definition}
  \label{def:structural-perms-derivatives}
  \uses{def:structural-perms}
  %leanok
...
\begin{definition}
  \label{def:near-litters-action}
  \uses{def:structural-perms-atoms,def:near-litters}
  $\Str_{-1}$ acts on near-litters.
\end{definition}

\begin{definition}
  \label{def:allowable-perms-atoms}
  \uses{def:structural-perms-atoms}
...
\begin{definition}
  ...
  \uses{def:near-litter-action, def:derivative, def:support-condition}
...
\begin{definition}
  ...
  \uses{def:allowable-perm, support}
...
\begin{lemma}
  ...
  \uses{def:rep-preserved}
  Allowable permutations act semi-tangles.
\end{lemma}
...
[This will be easy if we defined paths by induction from the top; if we have paths in reverse, it may need lemmas on how derivatives interact with extension at the top.  Or they might follow from functoriality of derivatives?]
\end{proof}
...
\begin{definition}
  ...
  \item an injection $\tau \to \mathrm{Pretangle}_\alpha$;
...
\begin{definition}
  ...
  \uses{def:semi-tangles, def:phase-1c-data}
  There is an injection $\tau_\alpha \to \mathrm{Pretangle}_\alpha$ (where $\tau_\alpha$ is constructed from the given phase 1 data).  Moreover, this commutes with the $\All_\alpha$-action.
\end{definition}
