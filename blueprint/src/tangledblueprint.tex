...
\leanok
\lean{con_nf.atom}
Define $\mathcal L$ as $ \lambda ^ 2 \times \mu$ and $\tau_{-1}$ as $\mathcal L \times \kappa$.  Note that both sets have size $\mu$.
\end{definition}
...
\begin{definition}
\label {def:ftargets}
\uses{def:local-cardinal, def:type-index, def:proper-type-index, def:fmap}
...
\uses{def:allowable, def:the-litter-near}
...
\uses{def:allowable, def:support}
...
\uses{def:tau-alpha, def:singletons}
