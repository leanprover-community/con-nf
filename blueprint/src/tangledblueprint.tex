...
\section{Phase 1a: Set codes and alternative extensions:  membership defined and extensionality enforced}
...
\uses{def:alt-ext, lem:f-map-properties}
\begin{enumerate}
...
\uses{def:semi-tangle, def:ainverse}
...
\section{Preliminaries on permutations and symmetry}

This section does not depend on any inductive assumptions.
...
\begin{definition}
  ...
  \uses{def:atoms,def:near-litter}
  \leanok
...
\begin{definition}
\label{def:support}
\uses{def:near-litter, def:derivative, def:support-condition-action}
...
[NOTES for formalisation: Well-orderings almost certainly shouldn’t be necessary in this phase. All this can also come before any inductive data are assumed, in the background on structural permutations.]
\end{definition}
...
\begin{lemma}
\label {lem:singletons-assumption}
\uses{def:representative-code}
...
[This is probably unnecessary now that we have the extrinsic construction of the groups of (pre-)structural perms.  If it is still needed, it should be included in the “phase 2 data” assumptions, as just assuming “typed singleton” embeddings $\tau_{-1} \to \tau_\beta$.]
\end{lemma}

\begin{definition}
\label {def:semiallowable-perm}
\uses{def:structural-perm, def:phase-1b-data}
...
\uses{def:code-action, lem:coherence-unpacked, def:representative-code}
...
\uses{def:rep-preserved,def:semi-tangle}
...
\begin{lemma}
  ...
  \uses{def:codes-action,def:equiv-codes}
...
\begin{proof}
Take a $\beta$-support for $b$, and extend all the paths in it at the top by the step $\beta < \alpha$.
...
\begin{definition}
\label{def:wellorder-conditions}
\uses{def:tangle}
[This doesn’t belong here in phase 1!  The ordering has been already assumed in the phase 1 data, and the technical conditions will need to be assumed later as part of the phase 2 data, at least by the proof of freedom-of-action; and then all of this will be constructed at the very end of phase 2.]

[Like most things about $<_\alpha$, this should probably be rephrased in terms of the injections $\iota_\alpha : \tau_\alpha \to \mu$.]

Let $\leq_\alpha$ be the assumed well-ordering of $\tau_\alpha$, needed for the recursive definition of the $f$ maps at later phases. This order needs to satisfy some technical conditions.
...
For each $x$ in $\tau_\alpha$ which is not a typed singleton or typed near-litter, we designate a support.  For any $(y,A)$ in the domain of the designated support of $x$, we must have $\iota_\alpha(y)<\iota_\alpha(x)$.
\end{definition}
...
\section{Preliminaries: coherent phase 2 data}

\begin{definition}
  \label{def:concrete-phase-1-data}
  \uses{def:phase-1a-data,def:phase-1b-data,def:phase-1c-data}
  For a type index $\alpha$, \emph{concrete phase 1 data} at level $\alpha$ consists of a subset $\tau \subseteq \Pretangle_\alpha$ and a subgroup $\All \subseteq G_\alpha$, together with the rest of phase 1a, 1b, and 1c data on these.
...
\begin{definition}
  ...
  \uses{def:concrete-phase-1-data,def:embedding-into-pretangles,def:near-litter-obligation,\def:allowable-perm}
  For $\alpha \in \lambda$, if we have concrete phase 1 data at all levels $\beta < \alpha$, then the constructions in phase 1 provide \emph{most} components of concrete phase 1 data at level $\alpha$ — everything except for the embedding $\iota_\alpha : \tau_\alpha \to \mu$!
...
\begin{definition}
  \label{def:phase-2-data}
  \uses{def:concrete-phase-1-data-propagates}
  For $\alpha \in \lambda + 1$, \emph{Phase 2 data up to $\alpha$} consists of concrete phase 1 data at every level $\beta < \alpha$, which is moreover \emph{coherent}, in that for each $\beta$, all components at level $\beta$ that \emph{could} have been induced from the data at lower levels $\gamma < \beta$ are, in fact, equal to what would have been induced that way.

  [Note we want to provide this definiton also for $\alpha = \lambda$!  I think this should be available in mathlib as \verb|with_top|.]

  For $\alpha < \beta$, “phase 2 data up to $\alpha$” can be restricted to “phase 2 data up to $\beta$”.

  Most constructions in phase 2 will assume phase 2 data up to $\alpha$, for some fixed $\alpha$.
\end{definition}
...
\uses{def:support-of}
[Note: all this stuff — raising and lowering indices, and the results about repeatedly applying them to structural permutations, etc — is MUCH more clearly described in algebraic language, with the category of paths, the functoriality of $\Str_\alpha$ and $\All_\alpha$ in paths, and maps between group actions, etc.]
...
\begin{theorem}
\label {lem:freedom-of-action}
\uses{def:perm-exception, lem:strengthen-support, def:local-bijection}
