...
\begin{definition}
  ...
  [Actually it’s probably simpler to go back to representing these as representative codes for now, and just define here the components $x_\beta$, for later embedding them into pretangles, which will become useful in phase 2.]
\end{definition}
...
\begin{definition}
  \label{def:structural-perms-near-litters-action}
  \uses{def:structural-perms-atoms}
...
\uses{def:structural-perm, def:equiv-code}
...
and $X$ is either a singleton of an element of $\tau_{-1}$ or a near-litter. [NOTE for formalisation: Well-orderings almost certainly shouldn’t be necessary in this phase. Also we should probably explicitly name the type of things that can appear in supports. This can be done before any inductive data are assumed.]
...
\uses{def:tangle}
[I'm not sure what to call this]  Let $\leq_\alpha$ be your favorite well-ordering of $\tau_\alpha$, needed for the recursive definition of the $f$ maps at later phases.
