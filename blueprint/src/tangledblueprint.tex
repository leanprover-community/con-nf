...
\lean{con_nf.struct_perm.mul_action_support_condition}
\leanok
...
\begin{lemma}
  ...
  \uses{def:support-of}
  \lean{con_nf.mk_small_support_le}
  \leanok
...
\label{lem:a-map-wf}
\uses{def:a-map}
\lean{con_nf.A_map_rel'_well_founded}
\leanok
...
\lean{con_nf.allowable_perm.smul_f_map}
\leanok
...
\uses{def:tangle}
\lean{con_nf.small_supported_singleton}
\leanok
...
\uses{def:tangle}
\lean{con_nf.small_supported_set}
\leanok
...
\begin{definition}
  ...
  \uses{def:tangle-data}
  \lean{con_nf.phase_2_assumptions}
  \leanok
...
\begin{definition}
  ...
  \uses{def:binary-condition}
  \lean{con_nf.struct_perm.to_spec}
  \leanok
...
\begin{definition}
  ...
  \uses{def:binary-condition}
  \lean{con_nf.struct_perm.satisfies}
  \leanok
...
\begin{proposition}
  ...
  \uses{def:struct-perm-graph,def:condition-functoriality}
  \lean{con_nf.struct_perm.spec_lower_eq_derivative}
  \leanok
...
\begin{proposition}
  ...
  \uses{def:condition-total-1-1,def:struct-perm-graph}
  \lean{con_nf.spec.graph_struct_perm}
  \leanok
...
\begin{definition}
  ...
  \uses{def:foa-context}
  \lean{con_nf.flexible}
  % not \leanok until we can verify the definition is actually completely true
...
\begin{definition}
  ...
  \uses{def:allowable-partial-perm,def:struct-perm-graph,def:struct-perm-extends}
  \lean{con_nf.freedom_of_action}
  \leanok
...
\begin{theorem}
  ...
  \lean{con_nf.freedom_of_action_propagates}
  \leanok
...
\begin{definition}
  ...
  \lean{con_nf.allowable_partial_perm.perm_le}
  \leanok
...
\begin{proposition}
  ...
  \lean{con_nf.allowable_partial_perm.allowable_Union}
  \leanok
...
\begin{definition}
  ...
  \uses{def:foa-context}
  \lean{con_nf.constrains}
  % not \leanok until we fix the wf condition!
...
\begin{proposition}
  ...
  \lean{con_nf.constrains_wf}
  \leanok
  The relation \( \prec \) is well-founded.
\end{proposition}
\begin{proof}
  % not \leanok until we fix the constrains relation to actually be wf
...
\begin{proposition}
  ...
  \lean{con_nf.spec.allowable.lower}
  \leanok
...
\begin{proof}
  ...
  % not \leanok until we fix lower_flexible_cond
...
\begin{proposition}
  ...
  \lean{con_nf.exists_tangle_of_supports}
  \leanok
...
\begin{lemma}
  ...
  \uses{def:allowable-partial-perm-order}
  \lean{con_nf.allowable_partial_perm.total_of_is_max}
  \leanok
...
\begin{proposition}
  ...
  \lean{con_nf.allowable_partial_perm.extends_to_allowable_of_total}
  \leanok
