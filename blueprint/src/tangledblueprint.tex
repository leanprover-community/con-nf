...
This should be read as an attempt at a recursion over the levels $\alpha \in \lambda$ of the structure.  We start by describing what data should exist at each level of the structure; then this section consists mostly of showing that given this data at each level $\beta < \alpha$, we can construct almost all this data at level $\alpha$.
...
For now, we group the data assumed at each level, according to which construcitons it will be needed for.
...
\begin{definition}
  ...
  \leanok
  Fix a proper type index $\alpha$.  \emph{Phase 1a data at level $\alpha$} consists of:
  ...
  \begin{definition}
    ...
    \lean{con_nf.phase_1b,con_nf.phase_1b_coherence}
    \leanok
    ...
    \item such that the map $j$ from near-litters to $\tau$ commutes with the $\All$-action.
    \end{itemize}
  ...
  \begin{definition}
    ...
    \lean{con_nf.phase_1}
    \leanok
    \emph{Full phase 1 data} at $\alpha$ consists of phase 1c data, together with:
    \begin{enumerate}
      \item a “typed singleton” injection $k : \tau_{-1} \to \tau$, commuting with the $\All$-action;
  ...
  When we assume some of this at multiple levels, we will refer to $\tau_\beta$, $\All_\beta$, etc.  Notice that some of this data also exists at level $-1$ — in particular, $\tau_{-1}$ and $\iota_{-1} : \tau_{-1} \to \mu$ — but not all of this data.
...
For the subsequent constructions of this section, we assume we are given phase 1c data for all $\beta < \alpha$.
...
\begin{definition}
  ...
  \uses{def:phase-1-data,def:embedding-into-pretangles,def:typed-near-litter,def:typed-singleton,lem:tangle-action}
...
\begin{definition}
  ...
  \uses{def:concrete-phase-1-data,def:phase-1-data-propagates}
