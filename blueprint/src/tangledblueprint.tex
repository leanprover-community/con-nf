...
\begin{definition}
\label {def:stage1-data}
\uses{def:pretangle, def:type-index}
\lean{con_nf.recursion_motor}
Fix a proper type index $\alpha$.

Assume that for each earlier index $\beta < \alpha$ we have the following data defined:
\begin{itemize}
  \item the set $\tau_\beta$;
  \item an injection $\iota_\beta : \tau_\beta \to \mu$ [alternatively, we could assume an ordering $<_\beta$ of order-type $\mu$]
  \item an injection $j_\beta$ from the set of near-litters to $\tau_\beta$. [These will be concretely represented as the \emph{typed near-litters} below.]
\end{itemize}

Call this \emph{stage 1 data} for level $\beta$.
\end{definition}
...
\begin{definition}
\label {def:code}
\uses{def:stage1-data}
%\leanok
\lean{con_nf.code}
An $\alpha$-code is a triple $(\alpha,\gamma,G)$ where $\gamma<\alpha$ and $G \subseteq \tau_\gamma$. (We include the $\alpha$ in case we later want to compare codes at different levels.)
\end{definition}

\begin{definition}
\label {def:fmap}
\uses{def:ftargets, def:code,de}
We define for $\beta$ a type index less than $\alpha$ and $\gamma$ a proper type index less than $\alpha$ and distinct from $\beta$, a map $f_{\beta,\gamma}$ from $\tau_\beta$ to $\mu$, as follows:

for $x \in \tau_\beta$, $f_{\beta,\gamma}(x)$ is the litter $[N]$ of the third component of the first $(\gamma,-1,N)$ in $<_\gamma$ such that $N$ is a near-litter and $[N] \in X_{(\beta,\gamma)}$ and $\iota_\gamma(\gamma,-1,M)>\iota_\beta(x)$ for each $M \in [N]$ and $[N] \neq f_{\beta,\gamma}(y)$ for any $y <_\beta x$.
\end{definition}

Notice that when we are constructing type $\alpha$, we assume that we have stage 1 data, which gives us the information already to define $f_{\beta,\gamma}$ for all $\beta, \gamma < \alpha$.  This definition will in the end not actually depend on the value of $\alpha$.

\begin{definition}
\label {def:typed-near-litter}
\uses{def:near-litter, def:code}
Codes $(\alpha,-1,N)$ where $N$ is a near-litter are called {\em typed near-litters\/}.  [These will eventually provide the injection $j_\alpha$ from near-litters into $\tau_\alpha$.]
\end{definition}
...
(Here $j_\delta(N)$ is the “typed near-litters” of level $\delta$, assumed in the stage 1 data.)
...
Note that the computation of $A_\delta$ does not depend on $\alpha$ apart from the fact that $\alpha$ must be larger than $\gamma$ and $\delta$.
\end{definition}
...
\begin{proof}
  Consider the map $j$ from codes to $\mu$ sending $(\alpha,\gamma,X)$ to $\min \iota_\gamma``X$, where $\iota_\gamma : \tau_\gamma \to \mu$ is the  position function assumed in the stage 1 data.
...
\begin{definition}
  ...
  [Note that these are intermediate between “pretangles” and “tangles”.]
\end{definition}

\begin{definition}
\label {def:ttt-membership}
\uses{def:tangle, def:ainverse}
Membership relations of $\alpha$-semi-tangles, for the intended model of tangled type theory, can now be defined as follows: for each proper type index $\beta < \alpha$, and $x \in \tau_\beta$, and each $\alpha$-pretangle $y \in \tau_\alpha$, say $x \in_{TTT} y$ just if $x \in y_\beta$.
\end{definition}
...
This section should have its inductive assumptions collected together as “stage 2 data”, or possibly “stage 1a data” if they can be specified abstractly without referring to the constructions relying on stage 1 data.
...
[This is probably not needed now that we have the extrinsic construction of the groups of pre-)structural perms.  If it is needed, it should be included in the “stage 2 data” assumptions, as just assuming “typed singleton” embeddings $\tau_{-1} \to \tau_\beta$.]
\end{lemma}
...
\uses{def:structural-perm, def:near-litter-perm, def:equiv-code}
...
\begin{definition}
\label {def:support}
\uses{def:near-litter, def:derivative}
...
\begin{lemma}
\label {lem:restrict-allowable-perm}
\uses{def:support-of}
